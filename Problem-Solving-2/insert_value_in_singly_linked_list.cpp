...
void insert_a_new_node(Node* &head, Node* &tail, int val) {
...
void insert_in_an_index(Node* &head, Node* &tail, int val, int inx, int &cnt, bool &ok) {
  if(inx > cnt || inx < 0)  {
    cout << "Invalid\n";
    ok = false;
    return;
...
void solve() {
  ...
  while(1) {
    ...
    if(x == -1) break;
    cnt++;
    insert_a_new_node(head, tail, x);
  }
  int e; cin >> e;
  while(e--) {
    bool ok = true;
    int m, n; cin >> m >> n;
    insert_in_an_index(head, tail, n, m, cnt, ok);
    if(ok) print_singly_linked_list(head);
  }
}
