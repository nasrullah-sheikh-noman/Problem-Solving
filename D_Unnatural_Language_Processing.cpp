...
void solve() {
  ...
  // cout << tmp << endl;
  for(int i = 0; i < n; i++) {
    if(tmp[i] == 'a' || tmp[i] == 'e') tmp[i] = 'v';
    if(tmp[i] == 'b' || tmp[i] == 'c' || tmp[i] == 'd') tmp[i] = 'c';
  }
  for(int i = 0; i < n; ) {
    if(i+2==n-1 && tmp[i]=='c' && tmp[i+1]=='v' && tmp[i+2]=='c') {
      cout << s[i] << s[i+1] << s[i+2] << nl;
      return;
    }
    else if(i+1==n-1 && tmp[i]=='c' && tmp[i+1]=='v') {
      cout << s[i] << s[i+1] << nl;
      return;
    }
    if(tmp[i]=='c' && tmp[i+1]=='v' && tmp[i+2]=='c' && tmp[i+3]=='c') {
      cout << s[i] << s[i+1] << s[i+2] << ".";
      i+=3;
    }
    else if (tmp[i]=='c' && tmp[i+1]=='v') {
      cout << s[i] << s[i+1] << ".";
      i+=2;
    }
  }
